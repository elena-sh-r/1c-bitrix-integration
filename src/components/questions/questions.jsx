          ...
          <div className={`${questionsStyles.questions__left}`}>
            ...
            <div className={`${questionsStyles.questions__container}`}>
              {data?.questions?.map((question) => {
                return (
                  <div key={question.id} className={`${questionsStyles['questions-item']}`}>
                    <div className={`${questionsStyles['questions-item__title']} ${open === question.id && questionsStyles['questions-item__title_active']}`} onClick={() => { return toggleHandler(question.id); }} onKeyUp={() => { return toggleHandler(question.id); }} role="button" tabIndex={0}>
                      {question.question}
                    </div>
                    <div className={`${questionsStyles['questions-item__answer']} ${open === question.id && questionsStyles['questions-item__answer_opened']}`}>
                      {question.answer}
                    </div>
                  </div>
                );
